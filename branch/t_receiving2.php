...
<html>
	...
	<body>
		<?php 
			...
			{
				$creator = $_SESSION['id'];
				$sqlquery = "INSERT INTO `transaction`( `Reference`, `Source`, `Destination`, `Comment`, `Date`, `Mark`, `Creator`, `Type`, `Code`) VALUES ( '', '200', '100', '', '$time_now', '2', '$creator', 1, '')";
				#echo $sqlquery;
			...
			{			
				$sqlquery3 = "UPDATE `particular` AS P,
								    `inventory` AS I,
								    `transaction` AS T 
								SET 
								    T.`Mark` = 1,
								    ...
								    I.`Mark` = 1
								WHERE
								    T.`ID` = '$sid' AND P.`Transaction` = '$sid'
								        ...
								        AND P.`ID` > 0
								        AND T.`ID` > 0";
				$finalize = mysqli_query($mysqli, $sqlquery3);
				#echo $sqlquery3;
				if($finalize)
				{
					unset($_SESSION['receive']);
			...
			if(isset($_POST['name']))
			{
				...
				$sqlquery = "INSERT INTO `inventory`(`Name`, `Category`, `Subcategory`, `Description`, `Weight`, `Buy`, `Sell`, `Mark`,`Code`) VALUES ('$name', '$category', '', '$description', '$weight', '$price', '0', '2', '$purchasecode')";
		...
		<div style="width:100%">
			<div class="row">
				...
				<div class="selecttable" style="width:98%;margin-left:1%">
					...
					<?php
						$sqlquery = "SELECT inventory.ID, Name, Category, Description, Weight, Amount, Code FROM particular, inventory WHERE particular.Transaction = '$sid' AND inventory.ID = particular.Inventory AND particular.Mark > 0";
						...
						echo '<th style="width:10%;text-align:center">Purchase Code</th>';
