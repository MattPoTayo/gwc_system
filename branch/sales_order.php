<?php 
	...
	if(isset($_GET['id']))
	{
		...
		if($check AND mysqli_num_rows($check) == 1)
		{
			...
			$dateactivity = date("F d, Y", strtotime($activity['Delivery_Date']));
					...
					<tr>
						...
						<td style='text-align:right;'>
							<p style='display:inline;font-size:20px;font-weight:bold;'>
								<?php
									$type = mysqli_query($mysqli, "SELECT Type FROM `sales_head` AS SH WHERE SH.`Mark` = 1 AND SH.`ID` = '$creation' LIMIT 1");	
							...
							$sqlQuery = @"SELECT 
										...
										FROM
										    `activity_detail` AS AD
										        LEFT JOIN
										    `particular_activity` AS PA ON PA.`Details` = AD.`ID`
										        LEFT JOIN
										    `brand` AS B ON B.`ID` = AD.`BrandID`
												LEFT JOIN
											`entity` AS E ON E.`ID` = AD.`ClientID`
												LEFT JOIN
											`activity_type` AS AT ON AT.`ID` = AD.`ActivityType`
												LEFT JOIN
											`contacts` AS C ON C.`ID` = AD.`ContactID`
												LEFT JOIN
											`supplier` AS S ON S.`ID` = AD.`SupplierID`
										WHERE
										    PA.`Activity` = '$creation'
										        AND PA.`Mark` > 0";
